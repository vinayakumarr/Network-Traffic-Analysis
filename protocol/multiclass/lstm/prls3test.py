...
model = Sequential()
model.add(LSTM(16,input_dim=23, return_sequences=True))  # try using a GRU instead, for fun
model.add(Dropout(0.1))
model.add(LSTM(16, return_sequences=True))  # try using a GRU instead, for fun
model.add(Dropout(0.1))
model.add(LSTM(16,input_dim=41, return_sequences=False))  # try using a GRU instead, for fun
model.add(Dropout(0.1))
model.add(Dense(11))
...
model.save("kddresults/lstm3layer/lstm1layer_model.hdf5")

loss, accuracy = model.evaluate(X_test, y_test)
print("\nLoss: %.2f, Accuracy: %.2f%%" % (loss, accuracy*100))
y_pred = model.predict_classes(X_test)
np.savetxt('kddresults/lstm3layer/lstm1predicted.txt', np.transpose([y_test1,y_pred]), fmt='%01d')
'''
...
y_test = y_train
# try using different optimizers and different optimizer configs
model.load_weights("kddresults/lstm3layer/checkpoint-892.hdf5")
#y_train1 = y_test
