...
model = Sequential()
model.add(SimpleRNN(4,init=lambda shape, name: normal(shape, scale=0.001, name=name),inner_init=lambda shape, name: identity(shape, scale=1.0, name=name),input_dim=22,activation='relu'))  # try using a GRU instead, for fun
model.add(Dense(11))
...
model.save("kddresults/lstm1layer/lstm1layer_model.hdf5")
