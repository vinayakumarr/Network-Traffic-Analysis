...
model = Sequential()
model.add(SimpleRNN(8,init=lambda shape, name: normal(shape, scale=0.001, name=name),inner_init=lambda shape, name: identity(shape, scale=1.0, name=name),input_dim=22,activation='relu',return_sequences=True))  # try using a GRU instead, for fun
model.add(SimpleRNN(8,activation='relu',return_sequences=False))
model.add(Dense(11))
...
model.save("kddresults/lstm2layer/lstm1layer_model.hdf5")

loss, accuracy = model.evaluate(X_test, y_test)
print("\nLoss: %.2f, Accuracy: %.2f%%" % (loss, accuracy*100))
y_pred = model.predict_classes(X_test)
np.savetxt('kddresults/lstm2layer/lstm1predicted.txt', np.transpose([y_test1,y_pred]), fmt='%01d')
