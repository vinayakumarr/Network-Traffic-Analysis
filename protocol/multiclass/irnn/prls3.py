...
model = Sequential()
model.add(SimpleRNN(16,init=lambda shape, name: normal(shape, scale=0.001, name=name),inner_init=lambda shape, name: identity(shape, scale=1.0, name=name),input_dim=22,activation='relu',return_sequences=True))  # try using a GRU instead, for fun
