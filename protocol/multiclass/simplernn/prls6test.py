...
model = Sequential()
model.add(SimpleRNN(64,input_dim=23, return_sequences=True))  # try using a GRU instead, for fun
...
model.add(SimpleRNN(64, return_sequences=True))  # try using a GRU instead, for fun
model.add(Dropout(0.1))
model.add(SimpleRNN(64,input_dim=41, return_sequences=False))  # try using a GRU instead, for fun
model.add(Dropout(0.1))
model.add(Dense(11))
...
np.savetxt('kddresults/lstm6layer/lstm1predicted.txt', np.transpose([y_test1,y_pred]), fmt='%01d')
'''
...
y_test = y_train
# try using different optimizers and different optimizer configs
model.load_weights("kddresults/lstm6layer/checkpoint-89.hdf5")
#y_train1 = y_test
#model.compile(loss='binary_crossentropy',optimizer='adam',metrics=['accuracy'])

y_pred = model.predict_classes(X_test)
np.savetxt('res/expected6.txt', y_test1, fmt='%01d')
np.savetxt('res/predicted6.txt', y_pred, fmt='%01d')
